...
func _build_interface() -> void:
	...
	var margin := UiKit.safe_margin(self, 18, int(profile.safe_top * 0.55), 10)
	...
	logo.add_theme_constant_override("separation", -22)
	root.add_child(logo)
	_add_status_row(logo)
	logo.add_child(_logo_line("POTION", 68, Color("d8ecff"), Color("315c9b")))
	logo.add_child(_logo_line("ROGUE", 78, Color("ffd17a"), Color("8c3d08")))
	var subtitle := UiKit.label("SORT PUZZLE RPG", 17, Color("eee0ff"))
	...
	hero.custom_minimum_size.y = 360 if profile.name == "tall" else 310
	...
	commands.add_child(upgrades)
	var settings := _command_button("SETTINGS", Color("7948aa"))
...
func _command_button(text: String, accent: Color) -> Button:
