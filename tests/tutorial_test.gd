...
func _ready() -> void:
	...
	director.configure()
	check(director.active and director.steps.size() == 14, "first run starts fourteen-step tutorial")
	...
	var actions := ["intro", "inspect_enemy", "inspect_intent", "select_source",
			"select_target", "undo", "complete_potion", "gain_mana", "observe_essence",
			"trigger_reaction", "observe_ultimate", "cast_skill", "learn_remix", "choose_path"]
